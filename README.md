...
_**NOTE**: as of July 2021, the ASW website no longer permits access via R. This limits the functionality of AmphiNom. The three functions that crawl the ASW website (`getTaxonomy()`,`getSynonyms()` and `aswSearch()`) will not work. Nonetheless, AmphiNom can still be used with the internally stored dataset, last updated on the 5th of July 2021._  

_As a result, installing the packages with vignettes will fail, make sure to disable these while installing (`build_vignettes = FALSE`). Versions of vignettes and tutorials can be accessed [here](https://hcliedtke.github.io/R-scrapheap/amphinom_example.html) and [here](https://hcliedtke.github.io/R-scrapheap/amphinom_tutorial.html)._ 
