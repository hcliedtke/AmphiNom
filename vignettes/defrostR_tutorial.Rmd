...
defrostR is a package written in the R programming language. The most recent version can be downloaded from [gitHub](https://github.com/hcliedtke/defrostR), using the 'devtools' library. The first step is therefore to make sure that the [devtools](https://cran.r-project.org/web/packages/devtools/index.html) package is installed and loaded. Successful installation of 'defrostR' might also require that the [XML](https://cran.r-project.org/web/packages/XML/index.html) package is installed prior.
