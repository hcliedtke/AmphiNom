---
title: "How to use AmphiSync for combining amphibia datasets"
...
vignette: >
  %\VignetteIndexEntry{How to use AmphiSync for combining amphibia datasets}
...
AmphiSync is a package written in the R programming language. The most recent version can be downloaded from ['gitHub'](https://github.com/hcliedtke/AmphiSync), using the 'devtools' library. The first step is therefore to make sure that the ['devtools'](https://CRAN.R-project.org/package=devtools) package is installed and loaded. Successful installation of 'AmphiSync' might also require that the ['XML'](https://CRAN.R-project.org/package=XML) package is installed prior.
...
Hopefully the most useful data-set this package generates is an up-to-date list of all possible synonyms per species, to facilitate matching data sets that might be using different names for the same species. As with the taxonomic table, a version of the current synonym list is stored in the AmphiSync package internally. This may of course be outdated and so to generate your own, up-to-date version run the getSynonym() function! The function takes the getTaxonomy() output as a reference and then looks through every species webpage to tabulate all listed synonyms. It is therefore important that you specify a "asw_taxonomy" object that is more up-to-date than the one stored in the package (if none is specified, AmphiSync will use the internal version). Generating a full list for all amphibian species can take a long, long time (up to an hour). More realistically, you may only be interested in a specific taxonomic group and so you can narrow down your search as in this example here:
...
For this tutorial however, we will use a version stored internally in AmphiSync as "amphweb" (again,might be out of date so be careful). 
...
```{r message=FALSE, warning=FALSE, results='hide'}
hyla.names<-defrost(query=paste(amphweb.hyla$genus, amphweb.hyla$species),asw = asw_synonyms)
...
this tells us that less than half of the names are up to date, and AmphiSync could update quite a few on its own without any complications. AmphiSync found all names on ASW and there were no issues with ambiguities (queries which mach synonyms belonging to more than one species), but the automatic updating has resulted in 4 cases where names are not unique anymore! These need to be checked carefully and most likely are units that used to be distinct and are now synonyms. We can get more of an idea of what has happened here by asking AmphiSync to spit out a bit more information:
